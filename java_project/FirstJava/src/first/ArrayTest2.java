...
public class ArrayTest2 {
	...
	public static void main(String[] args) {
		...
		// 2차원배열은 1차원 배열을 가지는 배열이다.
		System.out.println(arr.length);         // 2차원배열의 length값은 1차원 배열의 개수를 가짐
		                                        // length값은 2차원 배열의 행의 개수.		
		System.out.println(arr[0].length);      // arr[0]는 첫번째 1차원배열을 가리키는 주소값을 저장하는 변수
		System.out.println(arr[1].length);      // arr[1]는 두번째 1차원배열을 가리키는 주소값을 저장하는 변수
		System.out.println(arr[2].length);      // arr[2]는 세번째 1차원배열을 가리키는 주소값을 저장하는 변수

		
		System.out.println("2차원 배열의 요소 출력===================");
		showData(arr);
		//for 반복문을 이용해서 2차원 배열의 모든 요소를 참조
//		for(int i=0; i<arr.length; i++) { //0,1,2의 행값이 들어감.
//			for(int j=0; j<arr[i].length; j++) { // 0,1,2,3 열값
...
//	}
	...
	// for-each에서의 값의 변경 -> 배열에는 영향을 주지 않음.
	for(int n : arr4) {
		n++;
		System.out.println(n);
	}
