...
from emp join dept
-- on emp.deptno=dept.deptno
USING(deptno)  -- 공통 컬럼은 하나만 출력할 수 있게 한다. 공통 컬럼을 구별할 필요가 없다.
;

-- NATURAL JOIN : 동일 컬럼 조인의 조건을 생략, 같은 이름의 컬럼은 하나씩이어야 한다.
...
-- 중첩질의       – WHERE 부속질의, 단일열, 단순비교 -> 단일행, 집합 비교 -> 복합행
...
-- 스칼라 부속질의 : select절 프리젠테이션 영역에 쓸 수 있는 부속질의, 단일행, 단일열, 단일값의 결과만 나와야 한다.
select custid, (select name from customer where customer.custid=orders.custid) as "a",
       saleprice, orderdate
from orders
;
...
from orders o1
where saleprice > (select avg(saleprice) from orders o2 where o2.custid=o1.custid); -- custid=1이면, 1번 고객의 평균과 비교해서 큰 주문금액만 출력
