...
`pIdx` int not null unique primary key,
`name` varchar(10) not null,
`pNum` int not null,
`addr` varchar(100) default '한국',
...
insert into test.dept values ('50','MARKETING','LA');

-- 3. DEPT 테이블에 위에서 삽입한 데이터의 dname, loc 데이터를 변경하는 SQL을 작성하시오.
--   입력 데이터는 임의로 작성하시오.
update test.dept
set dname='DESIGN', loc='SAN FRANCISCO' where deptno=50;
...
create view emp_view as select * from emp e join dept d using (deptno);
select * from emp_view;
-- 3. EMP 테이블에서 모든 사원의 부서번호를 이름이 'SCOTT'인 사원의 부서번호로 변경하는 SQL을 작성하시오.
select deptno from emp where ename='scott';

update test.emp
set deptno=(select * from(select deptno from emp where ename='scott')as a);
