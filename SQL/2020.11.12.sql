...
-- AS 절을 추가하여 EMP 테이블과 동일한 내용과 구조를 갖는 EMP02 테이블을 생성해 봅시다.
create table emp02
as 
select * from emp;
desc emp02;
...
-- 테이블의 삭제 : 저장 공간을 삭제 -> 저장되어 있는 데이터도 모두 삭제
-- drop table table_name
drop table test_tbl;

-- TRUNCATE : 테이블의 모든 행을 바로 삭제, 물리적 적용도 바로 진행 DELETE
create table emp06
as
select * from emp;
select * from emp06;
...
-- 테이블 구조의 변경
-- alter table table_name {add(컬럼의 추가) | modify(컬럼의 수정) | drop(삭제)}
...
-- empno not nul, unique 제약을 동시에 적용
drop table emp03;
create table emp03(
...
create table emp09(
     ...
     CONSTRAINT emp09_deptno_fk FOREIGN KEY (deptno) REFERENCES dept(deptno)  -- 제약 조건의 정의 끝
);
...
-- 데이터 삽입
-- insert into table_name (데이터를 저장할 컬럼들) values(저장할 데이터들);
...
-- emp10 테이블의 데이터 중 부서 번호가 10번 부서의 사원을 삭제
